...
public class StationServiceImpl implements StationService {
    ...
    public Response update(Station info, HttpHeaders headers) {
        Optional<Station> opStation = repository.findById(info.getId());
    ...
    public Response queryById(String stationId, HttpHeaders headers) {
        Optional<Station> opStation = repository.findById(stationId);
        if (opStation.isPresent()) {
            return new Response<>(1, success, opStation.get().getName());
        } else {
